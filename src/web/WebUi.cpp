...
<h1>FPVCineCam32 <small>v0.10.8 ACTIVE SLOT TRACE</small></h1><div class=sub>Blackmagic + Betaflight MSP | active media slot diagnostic</div>
...
<div class=card><h3>Diagnostics</h3><pre id=status>Loading...</pre><button onclick=refresh()>Refresh</button><button onclick=clearCapture()>Clear 10:1 Trace</button><span id=clearMsg class=muted></span></div>
...
async function clearCapture(){
  try{await api('/api/clearCapture');el('clearMsg').textContent=' 10:1 trace cleared';setTimeout(()=>el('clearMsg').textContent='',1200);setTimeout(refresh,300)}
